...
class NewRaptor:
    ...
    def initGui(self):
        ...
        self.add_action(
            icon_path,
            text=self.tr(u'Add New Raptar Nest'),
            callback=self.run,
    ...
    def run(self):
        ...
        missing_layers=[]
        if not "raptor" in map_layers:
            missing_layers.append("raptor")
        if not "raptor_buffer" in map_layers:
            missing_layers.append("raptor_buffer")
        if not "linear_buffer" in map_layers:
            missing_layers.append("linear_buffer")
        if missing_layers:
            msg="The folowing layers are missing from this project\n"
        ...
        if result:
            ...
            lyrNests=QgsProject.instance().mapLayersByName("raptor")[0]
            lyrBuffer=QgsProject.instance().mapLayersByName("raptor_buffer")[0]
            lyrLinear = QgsProject.instance().mapLayersByName("linear_buffer")[0]
